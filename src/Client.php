	...
	function pushMetrics(string $job = null, string $instance = null)
	{
		...
		if ($instance && !$job) {
			throw new PrometheusException(
				"Instance passed but job was set to null.  Job must be set to a non empty string."
			);
		}
		if (!is_null($job) && $job == "") {
			throw new PrometheusException("Job was set to an empty string.  Job must be set to a non empty string.");
		} elseif (!is_null($instance) && $instance == "") {
			throw new PrometheusException(
				"Instance was set to an empty string.  If Instance is set it must be a non empty string."
			);
		}
